    ...
    <nav className="border-b border-slate-100 sticky top-0 bg-white/80 backdrop-blur-md z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link reloadDocument to="/" className="flex items-center gap-2 group transition-transform hover:scale-105">
            ...
            <span className="font-bold text-xl tracking-tight text-slate-900 group-hover:text-orange-600 transition-colors">Digital<span className="text-orange-600 group-hover:text-slate-900">Bareng</span></span>
          </Link>

          {/* Desktop Nav */}
          <div className="hidden md:flex items-center space-x-8">
            <Link reloadDocument to="/" className={`font-medium transition-colors ${location.pathname === '/' ? 'text-orange-600' : 'text-slate-600 hover:text-orange-600'}`}>Beranda</Link>
            <Link reloadDocument to="/guide" className={`font-medium transition-colors ${location.pathname === '/guide' ? 'text-orange-600' : 'text-slate-600 hover:text-orange-600'}`}>Guide</Link>
            <Link reloadDocument to="/tools" className={`font-medium transition-colors ${location.pathname === '/tools' ? 'text-orange-600' : 'text-slate-600 hover:text-orange-600'}`}>Tools</Link>
            <Link reloadDocument to="/blog" className={`font-medium transition-colors ${location.pathname.startsWith('/blog') ? 'text-orange-600' : 'text-slate-600 hover:text-orange-600'}`}>Blog</Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-slate-100 absolute w-full px-4 pt-2 pb-6 shadow-lg shadow-slate-200/20">
          <div className="flex flex-col space-y-4 text-center mt-4">
            <Link reloadDocument to="/" className="text-slate-700 font-medium text-lg py-2">Beranda</Link>
            <Link reloadDocument to="/guide" className="text-slate-700 font-medium text-lg py-2">Guide</Link>
            <Link reloadDocument to="/tools" className="text-slate-700 font-medium text-lg py-2">Tools</Link>
            <Link reloadDocument to="/blog" className="text-slate-700 font-medium text-lg py-2 border-b border-slate-100 pb-4">Blog</Link>
